# app_crypto.py — Crypto-only Smart Backtester
import os, time
...
# App config
# --------------------------------------------------------------------------------------
ASSET_FILTER = "crypto"   # hard filter
...
def attach_garch_risk_index(df_prices: pd.DataFrame,
    ...
    for _, r in trades_df.iterrows():
        ...
        if ann is None or not np.isfinite(ann) or ann <= 1e-9:
            out.append(np.nan); continue
        out.append(float(min(1.0, base_vol_dec/ann)))
    t2 = trades_df.copy()
    t2["garch_risk_index"] = out
    return t2

# --------------------------------------------------------------------------------------
# OAAT simulator (used by Tester/Compare)
# --------------------------------------------------------------------------------------
def simulate_oaat(
    ...
    *,
    ...
    if not taken:
        metrics = dict(final_capital=starting_capital, total_return=0.0,
                       trades=0, win_rate=float("nan"),
                       avg_win_ret=float("nan"), max_win=float("nan"), max_loss=float("nan"))
        return metrics, pd.DataFrame(columns=["symbol","entry_date","exit_date","entry","exit_or_last","ret_pct","status","capital_after"])
    ...
    metrics = dict(final_capital=capital, total_return=total_ret, trades=n_trades,
                   win_rate=win_rate, avg_win_ret=avg_win_ret, max_win=max_win, max_loss=max_loss)
    return metrics, res

# --------------------------------------------------------------------------------------
# Optimize per-symbol threshold (closed trades only)
# --------------------------------------------------------------------------------------
@st.cache_data(show_spinner=False)
def optimize_thresholds_per_symbol_closed(
    trades_all: pd.DataFrame, *, step: float = AUTO_GRID_STEP, min_trades: int = AUTO_MIN_TRADES
):
    ...
    for sym, cand in closed.groupby("symbol"):
        ...
        for thr in thr_values:
            ...
            rows.append({"symbol": sym, "threshold": thr, "trades": m["trades"],
                         "total_return_%": m["total_return"], "win_rate_%": (m["win_rate"]*100 if pd.notna(m["win_rate"]) else np.nan)})
...
def load_raw_prices(sig: str, asset_filter: str) -> pd.DataFrame:
    ...
    # clean sector to avoid 'None' chips in UI
...
df0 = load_raw_prices(stocks_sig_now, ASSET_FILTER)
...
if df0.empty:
    st.warning("No crypto rows found in stocks.csv.")
    st.stop()

# Universe (all cryptos present)
...
if trades.empty:
    st.warning("All trades were filtered out by auto thresholds.")
    st.stop()
...
# Ensure perf columns exist even if there are no closed trades yet
...
if page == "Home":
    ...
    if open_trades.empty:
        ...
        if pd.isna(earliest_entry):
            ...
            earliest_str = pd.to_datetime(earliest_entry).strftime("%Y-%m-%d")

        # best/worst safe guards
        best_row = open_trades.loc[open_trades["unrealized_pct_return"].idxmax()] if open_trades["unrealized_pct_return"].notna().any() else None
        worst_row = open_trades.loc[open_trades["unrealized_pct_return"].idxmin()] if open_trades["unrealized_pct_return"].notna().any() else None
        best_symbol = f"{best_row['symbol']} ({pct_str(best_row['unrealized_pct_return'])}) ✅" if best_row is not None else "—"
        worst_symbol = f"{worst_row['symbol']} ({pct_str(worst_row['unrealized_pct_return'])}) ❌" if worst_row is not None else "—"

        c1, c2, c3 = st.columns(3)
        c1.metric("Total Unrealized P/L", pct_str(unrealized_total))
        c2.metric("Open Trades", f"{len(open_trades)}")
        c3.metric("Earliest Entry", earliest_str)

        c4, c5, c6 = st.columns(3)
        c4.metric("Avg Holding (days)", "—" if pd.isna(avg_hold) else f"{avg_hold:.1f}")
        ...
        if not alarms.empty:
            ...
            st.dataframe(add_rownum(alarms_fmt[["Symbol","Sector","Entry Date","Entry","1D Change","P&L Now"]]),
...
if page == "Insights":
    ...
    if closed.empty:
        ...
        tot = (closed.groupby("symbol")["pct_return"].apply(total_ret_percent)
                      .rename("total_return_%").reset_index())

        win_mean  = (closed[closed["pct_return"] > 0].groupby("symbol")["pct_return"].mean()
                     .rename("avg_win_return"))
        loss_mean = (closed[closed["pct_return"] < 0].groupby("symbol")["pct_return"].mean()
        ...
        best["symbol_display"] = best["symbol"]  # no cap emojis for crypto
        ...
        st.download_button(
            ...
            file_name="all_trades_crypto.csv",
            mime="text/csv"
        )
...
if page == "Tester":
    ...
    selected_tickers = st.sidebar.multiselect("Optional ticker filter (within window)", options=sorted(tickers_in_window),
    ...
    metrics, res = simulate_oaat(candidates, start_ts, end_ts,
                                 starting_capital=starting_capital, alloc_pct=alloc_pct,
                                 threshold=0.50, require_garch=True)

    c1, c2, c3 = st.columns(3)
    c1.metric("Final capital", money_str(metrics["final_capital"]))
    c2.metric("Total return",  pct_str(metrics["total_return"]))
    c3.metric("Trades taken",  f"{metrics['trades']}")

    c4, c5, c6, c7 = st.columns(4)
    c4.metric("Max Win %",  pct_str(res["ret_pct"].max() if not res.empty else np.nan))
    c5.metric("Max Loss %", pct_str(res["ret_pct"].min() if not res.empty else np.nan))
    c6.metric("Avg Win %",  pct_str(res.loc[res["ret_pct"] > 0, "ret_pct"].mean() if not res.empty else np.nan))
    win_rate = (res["ret_pct"] > 0).mean() if not res.empty else np.nan
    c7.metric("Win Rate", "—" if pd.isna(win_rate) else f"{win_rate:.0%}")

    res_fmt = res.copy()
    res_fmt["ret_pct"] = res_fmt["ret_pct"].map(lambda x: pct_str(x))
    res_fmt["entry"] = res_fmt["entry"].map(money_str)
    res_fmt["exit_or_last"] = res_fmt["exit_or_last"].map(money_str)
    res_fmt["capital_after"] = res_fmt["capital_after"].map(money_str)
    res_fmt = date_only_cols(res_fmt, ["entry_date","exit_date"])
    st.dataframe(add_rownum(res_fmt), use_container_width=True, hide_index=True)
...
if page == "Compare":
    ...
    selected_tickers = st.sidebar.multiselect("Tickers (within window)", options=sorted(tickers_in_window),
                                              default=tickers_in_window)
    ...
    colD.metric("Δ Total return (B−A)", pct_str(metricsB["total_return"]-metricsA["total_return"]))
