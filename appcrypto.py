...
def add_rownum(df_in): df=df_in.copy(); df.insert(0,"#",range(1,len(df)+1)); return df
...
def _file_sig(path: str) -> str:
    try: s=os.stat(path); return f"{s.st_mtime_ns}-{s.st_size}"
    except FileNotFoundError: return f"missing-{time.time()}"
...
def attach_garch_risk_index(df_prices, trades_df, base_vol_dec: float = 0.20, lookback: int | None = 252):
...
@st.cache_data(show_spinner=False)
def optimize_thresholds_per_symbol_closed(trades_all, step: float = AUTO_GRID_STEP, min_trades: int = AUTO_MIN_TRADES):
...
# Sidebar (no Polygon update—crypto is updated by your GitHub workflow)
...
# Enrich (cap maps empty for crypto; symbol_display = symbol)
