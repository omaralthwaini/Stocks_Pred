...
# Universe: all crypto symbols from file
...
st.caption(
    f"Auto per-ticker thresholds applied (trained on CLOSED trades; "
    f"min_trades={AUTO_MIN_TRADES}, step={AUTO_GRID_STEP:.2f}). "
...
# Closed perf aggregates (ensure columns exist before the pages)
...
if page == "Home":
    ...
    if open_trades.empty:
        ...
        # safe sort even if avg_return is all NaN
        open_trades = open_trades.sort_values(["entry_date", "avg_return"],
        ...
        if not alarms.empty:
            ...
            alarms_fmt = alarms_fmt.rename(columns={
                ...
                "sector": "Sector",
                "entry_date": "Entry Date",
            })[["Symbol","Sector","Entry Date","Entry","1D Change","P&L Now"]]
