# app_crypto.py — Crypto-only Smart Backtester (hardened)
import os, time
...
def simulate_oaat(
    ...
    *,
    ...
    metrics = dict(final_capital=res["capital_after"].iloc[-1], total_return=total_ret,
                   trades=n_trades, win_rate=win_rate, avg_win_ret=avg_win_ret,
...
st.sidebar.caption("Source: stocks.csv (crypto rows only)")
...
# Ensure performance columns exist even if there are no CLOSED trades (avoid KeyError/None)
...
# Min/Max since entry (works for open + closed)
def add_minmax_for_trades(df_prices: pd.DataFrame, trades_df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _, tr in trades_df.iterrows():
        ...
        end = pd.to_datetime(tr["exit_date"], errors="coerce") if pd.notna(tr["exit_date"]) else last_date_by_sym.get(sym, pd.NaT)
        if pd.isna(start) or pd.isna(end): continue
        sl = prices[(prices["symbol"]==sym) & (prices["date"]>=start) & (prices["date"]<=end)]
        if sl.empty: continue
        rows.append({"symbol": sym, "entry_date": start, "min_low": float(sl["low"].min()), "max_high": float(sl["high"].max())})
    mm = pd.DataFrame(rows)
    return trades_df.merge(mm, on=["symbol","entry_date"], how="left")

trades = add_minmax_for_trades(df, trades)

# Re-ensure perf columns exist (in case of weird merges)
for col in ("avg_return","avg_win_return","win_rate","unrealized_pct_return","entry","stop_loss"):
...
if page == "Home":
    ...
    for col, default in (("entry_date", pd.NaT), ("avg_return", np.nan)):
        if col not in open_trades.columns:
            open_trades[col] = default

    # Safe sort only on present columns
    ...
    if open_trades.empty:
        ...
        for c in needed:
            if c not in open_trades.columns:
                open_trades[c] = np.nan
...
if page == "Insights":
    ...
    if closed.empty:
        ...
        tot = (closed.groupby("symbol")["pct_return"]
               .apply(total_ret_percent).rename("total_return_%").reset_index())
...
if page == "Compare":
    ...
    with st.expander("Show taken trades (A)"):
        tmp = resA.copy(); tmp["ret_pct"] = tmp["ret_pct"].map(lambda x: f"{x:+.2f}%")
    ...
    with st.expander("Show taken trades (B)"):
        tmp = resB.copy(); tmp["ret_pct"] = tmp["ret_pct"].map(lambda x: f"{x:+.2f}%")
        tmp = date_only_cols(tmp, ["entry_date","exit_date"])
        st.dataframe(add_rownum(tmp), use_container_width=True, hide_index=True)
